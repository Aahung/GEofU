...
if (count($rows) != 0){
	$verifyResult = true;
	$row = $rows[0];
	$sql_special = "SELECT * FROM ge WHERE code = '" .$row['geCode']. "'";
	$rows_special = $conn -> query ($sql_special);
	if (count($rows_special) == 0){
		$sql2 = "INSERT INTO ge (code, name, name_CN, lang, quiz, finalExam, 
				presentation, report, groupMember, updatedBy, lastUpdate) VALUES 
				( :geCode, :name, :name_CN, :lang, :quiz, :finalExam, 
				:presentation, :report, :groupMember, :updatedBy, :lastUpdate)
					)";
	}
	else {
